...
function onVideoCallControlRequest(dwUserId, dwErrorCode, dwFlags, dwParam, szUserStr)
{
    ...
    if(window.parent.window.openVideoCallConversation){
        var start = UserName.lastIndexOf('(');
        var name = UserName.substring(0, start);
        var id = UserName.substring(start + 1, UserName.length - 1);
        window.parent.window.openVideoCallConversation(id, name);
    };
}
...
function onVideoCallControlFinish(dwUserId, dwErrorCode, dwFlags, dwParam, szUserStr)
{
    try{
        ...
        // TODO 2016-05-27 huangchen 强制关闭，并重新初始化（html接口问题临时解决方案）
        CloseCall(dwUserId);
    }catch (e){
        ForSession("会话结束..."); // 提示层
        //ShowHallDiv(true); // 显示大厅
...
function CloseCall(dwUserId){
    var UserName = BRAC_GetUserInfo(dwUserId, USERINFO_NAME); // 用户姓名
    CloseCallByUserName(UserName);
}

function CloseCallByUserName(UserName){
    if (window.parent.window.closeVideoCallConversation) {
        setTimeout(function () {
            var start = UserName.lastIndexOf('(');
            var id = UserName.substring(start + 1, UserName.length - 1);
            window.parent.window.closeVideoCallConversation(id);
        }, 2000);
    }
}
